...
func createUserFile(username string, name string, password string, email string) {
	os.Mkdir("Users2/"+username, 0777)
	os.Mkdir("Users2/"+username+"/friends", 0777)
	os.Mkdir("Users2/"+username+"/posts", 0777)

	c.Lock()
	c.filelocks["Users2/"+username] = &sync.RWMutex{}
	c.Unlock()
	fmt.Println("creating lock: ", "Users2/", username, "in create user file")

	// make user friends with self
	self_file, self_err := os.Create("Users2/" + username + "/friends/" + username + ".txt")
	...
	// make file with name
	name_file, name_err := os.Create("Users2/" + username + "/name.txt")
	...
	password_file, password_err := os.Create("Users2/" + username + "/password.txt")
	...
	email_file, email_err := os.Create("Users2/" + username + "/email.txt")
...
func deleteUserFile(username string) {
	getLock("Users2/" + username)
	err := os.RemoveAll("Users2/" + username)
	check(err)
	c.Lock()
	c.filelocks["Users2/" + username].Unlock()
	c.Unlock()
	fmt.Println("unlocked", "Users2/" + username, "in delete user file")
}

// function adds a file with friend's name into user's friend directory
func addFriendForUser(username string, friend string) {
	getReadLock("Users2/" + username)
	getLock("Users2/" + username + "/friends/" + friend + ".txt")
	file, err := os.Create("Users2/" + username + "/friends/" + friend + ".txt")
	c.Lock()
	c.filelocks["Users2/" + username + "/friends/" + friend + ".txt"].Unlock()
	c.Unlock()
	defer file.Close()
	c.Lock()
	c.filelocks["Users2/" + username].RUnlock()
	c.Unlock()
	fmt.Println("read unlocking Users2/", username)
...
func removeFriendForUser(username string, friend string) {
	filename := "Users2/" + username + "/friends/" + friend + ".txt"
	getLock(filename)
	getReadLock("Users2/" + username)
...
func userExists(username string) bool {
	fmt.Println("in user exists")
	getReadLock("Users2/" + username)
	if _, err := os.Stat("Users2/" + username); os.IsNotExist(err) {
		c.Lock()
		c.filelocks["Users2/" + username].RUnlock()
		c.Unlock()
		return false
...
func passwordCorrect(username string, password string) bool {
	filename := "Users2/" + username + "/password.txt"
	getReadLock("Users2/" + username)
...
func addPostFile(username string, post string) {
	...
	filename := "Users2/" + username + "/posts/" + my_uuid.String() + ".txt"
	getReadLock("Users2/" + username)
...
// func to check if users2 are friends
func areFriends(username string, friend string) bool {
	retval := true
	getReadLock("Users2/" + username)
	if _, err := os.Stat("Users2/" + username + "/friends/" + friend + ".txt"); os.IsNotExist(err) {
		retval = false
	}
	c.Lock()
	c.filelocks["Users2/" + username].RUnlock()
...
// function for selecting posts for users2 based on their friends list
func createUserFeed(username string) []post {
	var user_feed []post
	getReadLock("Users2/" + username)
	friends, _ := filepath.Glob("Users2/" + username + "/friends/*")
	fmt.Println("friend list", friends)
	for _, friend := range friends {
		friend_name := friend[len("Users2/"+username+"/friends/") : len(friend)-4]
		fmt.Println("friends name", friend_name)
		getReadLock("Users2/"+friend_name)
		friend_posts, _ := filepath.Glob("Users2/" + friend_name + "/posts/*")
		...
		for _, fpost := range friend_posts {
			...
			if fpost != "Users2/"+friend_name+"/posts/.DS_Store" {
				getReadLock(fpost)
		...
		c.Lock()
		c.filelocks["Users2/"+friend_name].RUnlock()
		c.Unlock()
	}
	c.Lock()
	c.filelocks["Users2/" + username].RUnlock()
...
func deleteUser(username string) {
	...
	getReadLock("Users2/")
	users2, _ := filepath.Glob("Users2/*")
	c.Lock()
	c.filelocks["Users2/"].RUnlock()
	c.Unlock()
	for _, user := range users2 {
		// for mac OS, have to add this if statement to prevent opening .DS_Store file
		if user != "Users2/.DS_Store/friends/"+username+".txt" {
			...
			if _, err := os.Stat(user + "/friends/" + username + ".txt"); os.IsNotExist(err) {
				...
				c.Lock()
				c.filelocks["Users2/"+user].RUnlock()
				c.Unlock()
...
func main() {
	...
	if _, err := os.Stat("Users2/"); os.IsNotExist(err) {
		os.Mkdir("Users2/", 0777)
	}
